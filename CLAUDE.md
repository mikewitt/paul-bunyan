...
- **A `TaskHandle` parents only inside a `with`.** Only `__enter__` sets the ambient contextvar and only `__exit__` clears it, so a handle used bare has a parent but never becomes one. That is what makes an out-of-order `ContextVar.reset()` structurally impossible — a stale token silently reverts and loses the sibling span rather than raising. `.subtask()` takes its parent as `self` instead of reading ambient context, because contextvars propagate into asyncio tasks but **not** into a bare `threading.Thread`; it must also build the child's OTel context from `self._span`, or the log and span hierarchies disagree in exactly that threaded case.
- **Progress ticks are sampled; the `end` row is not.** `advance()` emits at most one record per 50ms. This is a shipping constraint, not a tuning knob: a tight loop produces ~85,000 records/s against a buffer that drains 10,000 per 200ms, so per-item ticks overflow lumberjack's own buffer and trip its own dropped-records warning — Principle 6 violated by lumberjack's own records — and the write-through plain renderer would print a line per item, which is the disease the package exists to cure. Nothing is lost because `progress_current` is **absolute**, not a delta, and `end()` writes the final count unsampled. Do not "fix" a bar that looks coarse by lowering the interval.
