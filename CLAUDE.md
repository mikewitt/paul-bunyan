...
6. **Lossy display, bounded store — and never corrupt a traceback.** The store→display path is deliberately lossy (that's the product: verbose logging in, concise progress out). The buffer→store path must not drop records, and that half is unconditional: a record that reached the handler and did not reach the store is a bug. What the store then *retains* is a separate question with a stated answer — a rolling window of the newest `retain` records, **1,000,000 by default**, because the store is `:memory:` unless the application says otherwise and an unbounded one is a leak with a schedule. Measured at 319 bytes a record: 304 MiB at the bound, ~3 GiB at ten times it, linear with no plateau. Eviction is by arrival order only, **never by content** — what is dropped is always the oldest and never the interesting, and no record is preferred over another. `retain=None` restores unbounded retention; a `retain` low enough that a burst is evicted between two redraws makes a bar under-count, which is why there is a floor, and which is the kind of imprecision Principle 10 licenses. Separately: a live TTY display must be torn down cleanly on exit *and* on unhandled exception, before Python's excepthook prints — otherwise the traceback gets mangled by cursor control or overwritten by a redraw, and its closing frame must be bounded by the terminal's height for the same reason. An `atexit` dump of the last N records is a cheap diagnostic on top of that.
...
- **`tests/tier1/` is the acceptance contract, and a failing tier-1 test means the change is wrong, not the test.** Never edit one to make CI pass. If you believe a tier-1 test is genuinely wrong, say so in the pull request and change it as its own commit with its own argument — never in the commit that made it fail. The tiers, the rules that enforce them and what tier 1 deliberately does not cover are in `tests/README.md`; they are not restated here, because a definition kept in two places drifts in one of them.
...
- `lumberjack.init()` — installs the handler, takes ownership of the root logger, starts the store and renderer(s), and states how much the store retains (`retain=`, default 1M records, `None` for unbounded).
- `Session` — the components one `init()` owns (handler, store, renderer, output mode, pump), the retention bound and the exact row count that enforces it, plus what `shutdown()` must put back (the root logger's prior handlers and level). `init()` and `teardown` share one instance rather than each keeping their own copies of its parts. The instance lives in `session.py`, and `current_session() is None` is the single "is lumberjack running" question. `teardown` deliberately keeps its *own* reference to it, as an install token: it is installed and uninstalled on its own lifecycle, and its whole test suite drives it with fakes and no `init()` at all.
- `LumberjackHandler` — `logging.Handler` subclass; writes to a bounded write buffer (`collections.deque`) with source attribution. The bound is real: overflow evicts unread records, so the handler counts them and teardown reports the total at exit. `drain()` is destructive, so `restore()` is its other half — a store write that raises puts the batch back at the *front* of the buffer for the next tick to retry, trimmed to the same ceiling and counting the trim as `dropped`. Without it a failing `append()` lost the batch silently, which is the one loss Principle 6 forbids (#77).
- `RecordStore` — pluggable interface (SQLite default/stdlib, DuckDB optional) behind the shared write buffer. Roughly: `append(records)` — **all-or-nothing per batch**, which the handler's retry depends on and which SQLite gets by rolling back rather than leaving a half-inserted transaction for the next commit to pick up — `recent(n | since)`, `count_by_template(window)`, `count_by_source(window)`, `count_by_source_since(after_id)`, `task_events_since(after_id)`, `evict(before | keep_last)`, `templates()`. One implementation, two SQL dialects, one parametrized test suite across installed backends. `recent(n)` returns the last n oldest-first, which is also what the exit dump reads — there is deliberately no separate `tail()`. `count_by_source_since()` is the one anything on a timer should call: it groups only rows past a watermark, so a redraw costs what arrived rather than what the store holds. `evict()`'s caller is `flush()`, via `Session.retain` — see Storage backends below. The session's row count stays exact without a `COUNT(*)` because the handler is the only writer and a trim leaves the store holding exactly `retain`, so the count is *assigned* rather than decremented by what `evict()` returned: subtracting looks equivalent and drifts negative on a store that already held rows this session never wrote.
...
- `renderers/plan.py` — **what one redraw was told to draw, decided before anything is drawn.** `plan_frame()` takes what the models returned and produces one frozen `Frame`: every row, its label, its numbers, its state, plus a `FrameCounts` answering "how many bars" — which is seven numbers, not one, because `sources` counts call sites and `loops` counts inferred loops and conflating them is the root mistake above. `RichProgressRenderer` becomes a mapper from a `Frame` onto rich `Task`s, and it imports no `rich`, so the whole of the display's policy is now computable — and tested — on the bare install. Six things stay rich's and are named in its docstring: glyph choice, total withdrawal, re-layout, the elapsed clock, the one-`Live` rule, and cropping. `RichProgressRenderer.counts()` is deliberately computed from rich's own task dicts rather than from the frame, so that asserting the two agree compares the screen against the plan rather than the plan against itself; teardown reports it at exit, which is the durable answer for a run nobody watched.
...
- `lumberjack.lint` — the instrumentation linter (#40), and the rung-2 mechanism rather than a diagnostic: `check(paths)` returns a `Report` of `Finding`s over the same `static.py` walk the display reads, so it can say *which* line to add and where without anyone reading this document. Reachable as `python -m lumberjack.lint`; there is deliberately no console script yet. Its open follow-ups are #64 (stage detection sees sibling loops, not a sequence of stage functions), #65 and #66.
...
Three refusals, and each is a refusal rather than a guess. A body that can **skip a line** has no stable order (`Loop.stable_order`), so `transform`'s conditional warning costs it the row — a determinate bar over it would show a *wrong* percentage, which Principle 10 does not license. A body with one call site is `1 of 1` forever, which fails the criterion that admitted it; `phases`' announcement line is that case. And no source on disk, or a template that failed the drift guard, leaves no ordinal to look up. In all three the loop row draws exactly as it did before.

**A fourth refusal, added later: fewer than two members a record can *reach*** (#83). An f-string destroys its template, so that site can never be the stage on show; counting it toward the minimum admitted a row on the strength of a member that could never fill it, and `1 of 2` forever fails the criterion exactly as `1 of 1` does. The column width still spans every site — admission asks "can this fill?", width asks "how wide might this get?", and only the first is a claim. A call inside a generator expression was the same defect by another route and was fixed upstream instead: `funcName` really is `<genexpr>` (comprehensions are inlined by PEP 709 and are not affected), so static now says so and the site leaves the body it was never a member of.

**"Can skip a line" is broader than "sits under an `if`", and reading it narrowly was a fail-open bug** (#81). Three unrelated mechanisms produce the same fact — a statement-level branch (`if`, `try`, `match`, and `with`, because `contextlib.suppress` and `open` are one node type); the deferred half of an expression (`a and log(...)`, a conditional expression, an `assert` message); and anything textually after a `break`, `continue` or `return` in the same body. The guard clause is the everyday one, and it used to pin a position row at `1 of 3` for the whole of a skipped iteration while the loop row above it advanced. Where the rule now errs it errs *closed* — a guard clause marks the rest of the body conditional even though those sites do fire together — and that cost was measured before it was accepted: 4 of 135 loops with log sites across 1,692 files change verdict, all of them single-site loops the position row already refuses, so **zero** admitted rows were lost.
...
- **Two bar kinds share one `Live`; they are never two started `Progress` objects.** `Progress.start()` starts a `Live` of its own, and rich allows only one live per console: the second becomes `_nested`, at which point its `refresh()` re-renders *the root's* renderable and returns, so its bars never draw (verified against rich 15.0.0, `Live.refresh`). Giving each its own `Console` is worse — two Lives writing cursor control to one stderr corrupt the frame. The working shape is rich's documented one: the renderer owns a single `Live` over a `Group` and **neither `Progress` is started**. The `Live` is constructed over `Group(task_progress, source_progress)`; the heartbeat is prepended per refresh by `_compose()`, because it is a `Text` rather than something rich can re-render from itself — same order on screen, one fewer durable member. Owning the `Live` is also what made **#28's bounded final frame** possible, and it is fixed: `close()` hides inferred rows past the terminal's height before stopping, because rich crops every live frame and then deliberately un-crops the last one. Exact bars keep their slots, which is the group-order rule applied to the one frame that never had it.
...
|---|---|---|
| `sqlite` (default) | stdlib | Zero required deps. Raw `sqlite3`, not an ORM. **WAL multi-writer works** — measured, see #110 — so multiprocessing needs no IPC queue for this backend. Python's `sqlite3.connect()` also supplies a 5s `busy_timeout` by default, which is why contention never raised in that spike. |
...
Retention: in-memory by default (`:memory:`), bounded to `init(retain=...)` — `DEFAULT_RETAIN` is 1M — and **enforced by `flush()`**, which is the one place that already holds the registry lock across a read and a write and already knows how many rows it just appended. It is amortised rather than paid per drain: the trim fires once the session has written `retain` plus a tenth, and the count that decides is exact because the handler is the only writer and a trim resets it to `retain`, which is what the store then holds. Not inside `append()`, which would put a `DELETE` inside the batch whose all-or-nothing guarantee `handler.restore()` depends on; not on a second timer, because retention is a function of volume rather than of time. Measured at the default bound over 1.1M rows: 222ms per trim, of which 14ms is resolving the cutoff and the rest is the range delete, once per 100,000 records, on the pump's thread. Eviction must maintain derived progress state incrementally, not by recomputing from the full window — satisfied for the bar model by `count_by_source_since()`, and any future reader on a timer owes the same. The three renderer modules whose cumulative counts are monotonic *by construction* were written for this caller and finally have it.
...
3. Multiprocessing-aware capture. **The WAL spike is done and the answer was yes (#110)**, so this is no longer an IPC phase: what remains is a `store_path=` on `init()` (the default `:memory:` cannot be shared at all), an `os.register_at_fork` handler, and an explicit `busy_timeout`. **Deferred behind Phase 4 — see execution order below.**
...
**Execution order is not phase order.** Phase 4 was built before Phase 3. Nothing in Phase 4 touches the write path — it adds *readers* — while Phase 3 changes how records get into the store, so there is no collision and no rework. Multiprocessing capture is also infrastructure whose payoff is *more bars*, which is worth little until the bars themselves are worth multiplying. The numbers and milestones are deliberately left as they are: renumbering would invalidate "Phase 4" in every issue body that already cites it, for no gain. **The SQLite WAL multi-writer question was a one-day spike, not a phase, and it has been run — the answer is in #110.** Sixteen processes on one file store, 800,000 rows, nothing lost and no lock errors; the failure mode is latency rather than loss, and the retry `handler.restore()` already performs (#77) is exactly what a contended store needs. So most of Phase 3 did evaporate. What did *not*: `fork()` is a real defect — a child inheriting its parent's connection mid-transaction loses its first batch **silently**, which Principle 6 forbids, and reproduces three times in three. It needs no multiprocessing feature to reach, so it is filed as #111 against Polish rather than waiting for the phase.
...
| Phase | IN | OUT |
|---|---|---|
| **3 — Multiprocessing capture** | Records from child processes reaching the parent's store. The spike said WAL holds (#110), so: a `store_path=` on `init()`, an `os.register_at_fork` handler, an explicit `busy_timeout`, and whether `retain` is per-store or per-writer. The attribution columns already exist and need nothing. | Any display change — a second process is already a second set of bars. An IPC/queue layer for SQLite, which the spike retired. DuckDB multi-process — single-writer, so the queue path is still its answer. Anything networked. |
