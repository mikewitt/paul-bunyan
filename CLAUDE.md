...
**Trunk's required checks are `lint`, `typecheck`, the six `test` legs, `bare install (no rich)`, and CodeQL's `Analyze (actions)` / `Analyze (python)` — eleven.** Two checks that run are deliberately *not* required, and only one of those looks intentional: `coverage` should not block a merge, but `package` arguably should. It is the only check that exercises the artefact users actually install — it builds the wheel, installs it into a clean venv, and asserts `py.typed` ships — so a change that breaks packaging currently merges green.
