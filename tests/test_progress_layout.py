...
def test_nothing_nested_keeps_arrival_order():
    """Two unrelated loops are drawn in the order they started, which is the
    one thing first-qualified order was always right about."""
    order = [_src(1), _src(2), _src(3)]
    assert depth_first_order(order, {}) == order
...
def test_a_row_parented_by_itself_is_not_lost():
    lone = _src(6)
    assert depth_first_order([lone], {lone: lone}) == [lone]
